  ...
  initializePresetRoute: function (app) {
    app.get("/preset", async (req, res) => {
      ...
      const query = await db.get(
        `SELECT RatingMetric, CookingTimeMetric, CommonIngredientsMetric, NumberOfIngredientsMetric FROM Presets WHERE UserID=${req.query.userID} AND Name='${req.query.name}';`,
      );
      if (query === undefined) {
